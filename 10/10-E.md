...
```
types {
    tipo_mimetype1 extension1;
    tipo_mimetype2 extension2 [extension3...];
    [...]
}
```
Cuando Nginx sirve un archivo, verifica la extensión del archivo para determinar el tipo MIME. El tipo MIME se envía como el valor del encabezado HTTP Content-Type en la respuesta. Este encabezado puede afectar la forma en que los navegadores manejan los archivos. Por ejemplo, si el tipo MIME del archivo que estás solicitando es application/pdf, tu navegador podría intentar renderizar el archivo utilizando un complemento asociado con ese tipo MIME en lugar de simplemente descargarlo.
...
include mime.types;
```
Este archivo ya cubre las extensiones de archivo más importantes, por lo que probablemente no necesites editarlo. Si la extensión del archivo servido no se encuentra dentro de los tipos listados, se usa el tipo predeterminado, como lo define la directiva default_type (ver la siguiente entrada).

Ten en cuenta que puedes anular la lista de tipos volviendo a declarar el bloque de tipos. Un ejemplo útil sería forzar la descarga de todos los archivos en una carpeta en lugar de mostrarlos:
```
http {
    include mime.types;
    [...]
    location /descargas/ {
        # elimina todos los tipos MIME
        types { }
...
```
Algunos navegadores ignoran los tipos MIME y aún pueden mostrar archivos si su nombre de archivo termina con una extensión conocida, como .html o .txt.

Para controlar la forma en que los archivos son manejados por los navegadores de tus visitantes de manera más segura y definitiva, debes hacer uso del encabezado HTTP Content-Disposition a través de la directiva add_header, detallada en el módulo de encabezados HTTP (Capítulo 4, Configuración del Módulo).

Los valores predeterminados, si el archivo mime.types no está incluido, son los siguientes:
```
types {
    text/html html;
    image/gif gif;
