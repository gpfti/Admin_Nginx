...
3. **Creación de directorios y archivos:**
    ...
    Para cada uno de los siguientes archivos, crea una página web elemental que muestra su finalidad.
...
5. **Configuración de páginas de ejemplo:**
   Añade contenido HTML básico a los archivos `index.html` y `404.html` en cada directorio.

6. **Verificación de la configuración de Nginx:**
   ```
   sudo nginx -t
   ```

7. **Recarga de la configuración de Nginx:**
   ```
   sudo systemctl reload nginx
   ```

8. **Verificación del servidor:**
   Abre un navegador web e ingresa las direcciones `http://dominio1.com` y `http://dominio2.com` para verificar que el servidor esté sirviendo las páginas correctamente. Intenta acceder a rutas como `http://dominio1.com/images/` y `http://dominio2.com/downloads/` para asegurarte de que las rutas de alias estén funcionando correctamente.

Con estos pasos, has configurado un servidor web básico utilizando Nginx en Ubuntu 20.04, aplicando las directivas `listen`, `server_name`, `sendfile`, `root`, `alias`, `error_page`, `index` y `default_type`.
