...
Los bloques se consideran directivas que agrupan otras directivas. Sirven para categorizar/organizar el tipo de configuración a la que hacen referencia.
...
`tcp_nopush` y `tcp_nodelay` no se usan a la vez por cada conexión, son mutuamente excluyentes en el mismo socket.:  
- nopush: agrupa → espera
- nodelay: no espera → envía ya

Entonces: ¿Por qué están los dos en on entonces?
Porque Nginx los usa de forma inteligente y secuencial, según el contexto de cada respuesta:

🔹 Usa tcp_nopush on; cuando:
Estás sirviendo archivos grandes con sendfile.

Nginx agrupa los datos para enviarlos más eficientemente.

🔹 Luego activa tcp_nodelay on; cuando:
Ya terminó de enviar el archivo estático.

Pasa a manejar otras respuestas pequeñas (headers, ACKs, etc.).
...
## Creación de un servidor en archivo dedicado a un sitio
