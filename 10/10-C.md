...
Si los modulos están en: `/etc/nginx/modules-enabled/`, es que los has habilitado mediante un enlace blando (ejemplo linux) desde el sistema operativo.
...
Aunque, como veremos, cuando haces un: `include /etc/nginx/modules-enabled/*.conf;` en nignx.conf (como se hace habitualmente y veremos veremos), estás incluyendo todos los modulos que has habilitado.
...
En este log se registran todos los errores y advertencias. Esta directiva indica que la ruta del archivo donde se almacenarán estos registros es **`/var/log/nginx/error.log`**.  
...
Explicado con más detalle en: 
[Explicado con más detalle en la siguiente sección](10-D.md)
