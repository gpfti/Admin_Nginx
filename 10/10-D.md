...
```nginx
# --- Core Module ---
worker_processes 1; # normalmente "auto"
...
http {
    ...
    server {
     ...
     # --- HTTP Access Module ---
     # Control de acceso (permitir o denegar IPs)
     location /admin {
...
- `Sistenma de Configuration (Responsabilidad del nucleo de Nginx)`: Habilita el mecanismo de inclusión.
...
## Sistenma de Configuration (Responsabilidad del nucleo de Nginx).
...
## Módulo HTTP Core.
Incluye las directivas:
- server: Define la configuracion de un servidor virtual (puede haber varios)
- location: Indicacion la configuracion en funcion de cada direccion solicitada (indicada a continuacion de location)
- listen: Indica la dirección IP y el puerto en el que el servidor deberá escuchar.
- server_name: Inidca el nombre del servidor virtual.
